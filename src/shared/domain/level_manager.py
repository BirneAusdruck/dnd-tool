...
from src.shared.repositories.srd_repository import get_class
from src.shared.domain.character_builder import modifier


def _spell_slots_at(cls: dict | None, level: int) -> list[int]:
    if not cls or not cls.get("spellcasting"):
        return []
    slots = cls["spellcasting"]["slots"]
    return list(slots[min(level - 1, len(slots) - 1)])
...
def get_level_up_info(char_data: dict, class_index: str | None = None) -> dict:
    ...
    if cls and cls.get("spellcasting"):
        ...
        if multiclass:
            ...
            old_slots = _spell_slots_at(cls, class_level)
            new_slots = _spell_slots_at(cls, new_class_level)
