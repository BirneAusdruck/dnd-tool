...
class LevelUpDialog(QDialog):
    ...
    def __init__(self, char_data: dict, class_index: str | None, parent=None):
        ...
        self._info = LevelUpService.get_level_up_info(char_data, class_index)
        ...
        cls_name = (ClassService.get_raw(self._info["class_index"]) or {}).get(
    ...
    def _build_asi_feat_section(self) -> QGroupBox:
        ...
        grid = QGridLayout()
        for i, ab in enumerate(SRDConstants.ABILITIES):
        ...
        for feat in FeatService.get_all_raw():
    ...
    def _on_feat_selected(self, current: QListWidgetItem, _prev) -> None:
        if not current or not self._feat_detail:
            return
        feat = FeatService.get_raw(current.data(Qt.ItemDataRole.UserRole))
        if not feat:
            return
...
class LevelDownDialog(QDialog):
    ...
    def __init__(self, char_data: dict, parent=None):
        super().__init__(parent)
        info = LevelUpService.get_level_down_info(char_data)
        cls_name = (ClassService.get_raw(info["class_index"]) or {}).get(
        ...
        if info.get("feat_undone"):
            feat_name = (FeatService.get_raw(info["feat_undone"]) or {}).get("name", info["feat_undone"])
...
class ClassPickerDialog(QDialog):
    ...
    def _build_ui(self, char_data: dict) -> None:
        ...
        for cls in ClassService.get_all_raw():
            idx = cls["index"]
