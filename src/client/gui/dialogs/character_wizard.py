...
class _Step0_BasicInfo(QWidget):
    def __init__(self, parent=None):
        ...
        for a in SRDConstants.ALIGNMENTS:
...
class _Step1_Race(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._races = RaceService.get_all()
        self._build()
    ...
    def _on_subrace_changed(self, current, _prev):
        ...
        if not race_item:
            return
        race = RaceService.get(race_item.data(Qt.ItemDataRole.UserRole))
        if not race:
            return
...
class _Step2_Class(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._classes = ClassService.get_all_raw()
        self._build()
    ...
    def _on_class_changed(self, current, _prev):
        if not current:
            return
        cls = ClassService.get_raw(current.data(Qt.ItemDataRole.UserRole))
        if cls:
...
class _Step3_AbilityScores(QWidget):
    ABILITIES = SRDConstants.ABILITIES
    ...
    def _build(self):
        ...
        for i, ab in enumerate(self.ABILITIES):
            ...
            cb = QComboBox()
            for v in CharacterBuilderService.STANDARD_ARRAY:
                cb.addItem(str(v))
        ...
        # Point buy
        self.pb_grp = QGroupBox(f"Point Buy: {CharacterBuilderService.POINT_BUY_BUDGET} Punkte")
    ...
    def _update_mod_labels(self):
        for ab in self.ABILITIES:
            ...
            m = CharacterBuilderService.modifier(score)
            sign = "+" if m >= 0 else ""
    ...
    def _update_pb(self):
        total_cost = sum(CharacterBuilderService.POINT_BUY_COSTS.get(sp.value(), 0) for sp in self.pb_spins.values())
        remaining = CharacterBuilderService.POINT_BUY_BUDGET - total_cost
        color = COLORS["success"] if remaining >= 0 else COLORS["error"]
        self.pb_total_lbl.setText(f"Verwendete Punkte: {total_cost} / {CharacterBuilderService.POINT_BUY_BUDGET} (verbleibend: {remaining})")
        self.pb_total_lbl.setStyleSheet(f"color:{color};font-weight:bold;")
        for ab, sp in self.pb_spins.items():
            cost = CharacterBuilderService.POINT_BUY_COSTS.get(sp.value(), 0)
    ...
    def validate(self) -> str | None:
        if self._method == "standard":
            ...
            if selected != sorted(CharacterBuilderService.STANDARD_ARRAY):
                return "Jeder Wert des Standard-Arrays muss genau einmal vergeben werden."
        elif self._method == "pointbuy":
            total = sum(CharacterBuilderService.POINT_BUY_COSTS.get(sp.value(), 0) for sp in self.pb_spins.values())
            if total > CharacterBuilderService.POINT_BUY_BUDGET:
                return f"Point Buy: {total} Punkte verbraucht, Maximum ist {CharacterBuilderService.POINT_BUY_BUDGET}."
...
class _Step4_Background(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._backgrounds = BackgroundService.get_all()
        self._build()
    ...
    def _on_bg_changed(self, current, _prev):
        if not current:
            return
        bg = BackgroundService.get(current.data(Qt.ItemDataRole.UserRole))
        if bg:
...
class _Step5_Skills(QWidget):
    ...
    def update_for_class_and_bg(self, class_index: str, bg_skills: list[str]):
        ...
        cls = ClassService.get_raw(class_index)
        if not cls:
            return
        ...
        count = sc["count"]
        options = sc["from"]
        all_skills = [s["name"] for s in SRDConstants.SKILLS]
        ...
        col_len = (len(SRDConstants.SKILLS) + 1) // 2
        for i, skill in enumerate(SRDConstants.SKILLS):
            col = i // col_len
            row = i % col_len
...
class _Step6_Spells(QWidget):
    ...
    def update_for_class(self, class_index: str):
        ...
        if n_cantrips > 0:
            ...
            cantrips = SpellService.get_cantrips_raw(class_index)
        ...
        if n_spells > 0:
            level1_spells = SpellService.get_spells_raw(class_index=class_index, level=1)
    ...
    def validate(self) -> str | None:
        cls = ClassService.get_raw(self._class_index) if self._class_index else None
        sc = cls.get("spellcasting") if cls else None
        if not sc:
            return None
        cantrips_known = sc.get("cantrips_known", [])
...
class CharacterWizard(QDialog):
    ...
    def _go_next(self):
        ...
        if self._current == 0:
            set_edition(self.step0.edition())
            self.step1.reload()
            self.step2.reload()
            self.step4.reload()
            ...
            bg_idx = self.step4.selected_background()
            cls_idx = self.step2.selected_class()
            bg = BackgroundService.get(bg_idx) if bg_idx else None
    ...
    def _finish(self):
        ...
        data = CharacterBuilderService.build_character_data(
            name=name,
    ...
    def _apply_starting_level(self, data: dict, target_level: int) -> dict | None:
        ...
        for lvl in range(2, target_level + 1):
            ...
            info = LevelUpService.get_level_up_info(data, class_index)
            ...
            if not lvl_dlg.exec():
                return None
            data = LevelUpService.apply_level_up(
